...
```
사실 위 코드는 표현력이 좋은 코드는 아닙니다. 이유는 다음과 같습니다.
```javascript
const 빼기_20_함수 = 빼기_함수(20);
console.log(빼기_20_함수(50)); // 30 or -30 그것이 문제로다.
```
커링을 적용한 함수는 평가 시점을 조정하기 위해 `빼기_함수(50)(20)` 이렇게 연속적으로 실행하지 않습니다. 일반적으로 `빼기_함수(20)` 여기까지 실행 후 원하는 시점에 `빼기_20_함수(50)` 이렇게 평가를 수행하게 됩니다. 
위 경우 `빼기_20_함수(50)` 실행 결과는 50에 20 을 빼는 듯한 느낌을 개발자에게 줍니다. 다른 방법이 없을까요?
